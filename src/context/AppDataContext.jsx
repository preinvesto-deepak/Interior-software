...
function AppDataProvider({ children }) {
  ...
  // ── Initial load from MySQL (via server/state.php) ──────────────────────
  useEffect(() => {
    let cancelled = false;
    fetchState()
      .then((data) => {
        if (cancelled) return;
        setProjects(data.projects?.length ? data.projects : defaultData.projects);
        setSubProjects(data.subProjects?.length ? data.subProjects : defaultData.subProjects);
        setDimensions(data.dimensions?.length ? data.dimensions : defaultData.dimensions);
        setPrices(data.prices?.length ? data.prices : defaultData.prices);
